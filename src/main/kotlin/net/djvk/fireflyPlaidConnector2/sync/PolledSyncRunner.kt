...
class PolledSyncRunner(
    ...
    /**
     * Only intended for testing, this variable changes the behavior when a sync cursor is missing
     *  for an access token.
     * Normally, the connector discards all Plaid transactions from the sync endpoint up until now for
     *  the missing access token to essentially fast-forward the sync cursor state to now.
     * This variable changes that by processing these transactions normally.
     * This is intended to help with regression testing the polling functionality, which is currently a pain
     *  in the butt because you need a file with valid cursors for your test access tokens, which is not always
     *  easy to get (for instance, after you just migrated from the development to production environment).
     */
    @Value("\${fireflyPlaidConnector2.polled.ignorePastDataForNullCursors:true}")
    private val ignorePastDataForNullCursors: Boolean,
    ...
    override fun run() {
        runBlocking {
            ...
            mainJob = launch {
                ...
                if (ignorePastDataForNullCursors) {
                    ...
                    for ((accessToken, _) in accountAccessTokenSequence) {
                        ...
                        do {
                            val response =
                                executeTransactionSyncRequest(true, accessToken, cursorMap[accessToken], plaidBatchSize)
                                // This should never happen because the called function should throw an exception in this case.
                                    ?: throw RuntimeException("Exception logic failure")
                            logger.debug(
                 ...
                 */
                do {
                    fetchAndUpdateTransactions(accountMap, accountAccessTokenSequence, cursorMap)
    ...
    /**
     * Function that does the meat of the work, fetching Firefly and Plaid transactions, converting them,
     *  and upserting them into Firefly.
     */
    suspend fun fetchAndUpdateTransactions(
    ...
    /**
     * @param initialFetch Set to true if this is an initial "catch up" sync for a null cursor.
     *  Set to false for normal operation.
     *  This parameter changes the error handling behavior if [allowItemToFail] is true; specifically
     *  [allowItemToFail] only makes sense for ongoing sync calls, not an initial "catch up" sync.
     */
    suspend fun executeTransactionSyncRequest(
        ...
        try {
            ...
            val initialText = if (initialFetch) "initial" else "ongoing"
            logger.error("Error requesting $initialText Plaid transactions. Request: $request; ")
            if (allowItemToFail && !initialFetch) {
